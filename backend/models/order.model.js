...
OrderSchema.path("dealer").schema.pre("validate", function (next) {
  ...
  if (!hasPhone && !hasTelegram) {
    this.invalidate(
      "dealer.info.phoneNumber",
      "Either phone number or telegram ID is required",
    );
    this.invalidate(
      "dealer.info.telegramId",
      "Either phone number or telegram ID is required",
    );
  }
  next();
...
// Handle deleted and bin status in queries
OrderSchema.pre(
